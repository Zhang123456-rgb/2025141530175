...
def fetch_url():
    ...
    if not target_url:
        return render_template("index.html", fetch_status="错误", fetch_content="请输入URL")

    result = {"status": "未知", "content": ""}

    try:
        # ⚠️ SSRF漏洞：直接请求用户提供的URL，无任何限制
...
def ping():
    ...
    if request.method == "POST":
        ...
        if ip_input:
            # ⚠️ 命令注入：使用 f-string 拼接 + shell=True
