...
class P(FPDF):
    ...
    def img(self,path,w=170):
        if path:
            try: self.image(path,w=w)
            except: self.t(f"[图片加载失败: {path}]")
        self.ln(3)
...
for x in ["一、SQL注入漏洞来源分析","    1.1 什么是SQL注入","    1.2 漏洞代码定位",
          "二、SQL注入攻击测试与截图","    2.1 登录页面","    2.2 正常搜索",
          "    2.3 永真条件注入 (%' OR 1=1)","    2.4 UNION注入获取密码",
          "    2.5 后台SQL日志截图","三、漏洞危害分析",
          "四、修复方案","    4.1 参数化查询原理","    4.2 修复前后代码对比",
          "    4.3 修复后完整代码","    4.4 修复后SQL日志截图","五、修复验证测试",
          "六、访问地址"]:
...
pdf.h2("1.1 什么是SQL注入")
pdf.t("SQL注入（SQL Injection）是指攻击者在输入框中插入恶意SQL代码，利用后端对用户输入的不当处理，使恶意SQL语句被执行。OWASP Top 10中危害最大的漏洞之一。")
pdf.h2("1.2 漏洞代码定位")
...
pdf.c(
    ...
    'c.execute(sql)    # UNSAFE! No parameterization'
)
...
pdf.n("用户输入直接嵌入SQL语句，无任何过滤或转义！")

# ===== 二 =====
pdf.add_page(); pdf.h1("二、SQL注入攻击测试与截图")
pdf.h2("2.1 登录页面")
pdf.img("/root/flask-app-vuln/static/ss_login.png",w=170)
pdf.t("登录页面，使用 admin/admin123 登录系统。")

pdf.h2("2.2 正常搜索")
pdf.t("请求：/search?keyword=admin")
pdf.c(
    'Generated SQL:\n'
    "SELECT ... WHERE username LIKE '%admin%' OR email LIKE '%admin%'\n"
    'Result: returns admin user record'
)
pdf.img("/root/flask-app-vuln/static/ss_search_normal.png",w=170)

pdf.add_page()
pdf.h2("2.3 永真条件注入 (%' OR 1=1 --)")
pdf.t("攻击原理：闭合LIKE引号后添加 OR 1=1 使条件永真，-- 注释后续SQL。")
pdf.c(
    'URL: /search?keyword=%25%27+OR+1%3D1+--\n'
    'Decoded: %\' OR 1=1 --\n'
    '\n'
    'Generated SQL:\n'
...
pdf.n("结果：OR 1=1 使 WHERE 条件永真，返回 users 表所有记录！")
pdf.img("/root/flask-app-vuln/static/ss_inject_or.png",w=170)

pdf.h2("2.4 UNION注入获取密码")
pdf.t("攻击原理：使用 UNION SELECT 合并查询结果，读取 password 字段。")
pdf.c(
    'URL: /search?keyword=%27+UNION+SELECT+id,username,password,email+FROM+users+--\n'
    '\n'
    'Generated SQL:\n'
    "SELECT ... WHERE username LIKE '%' UNION SELECT\n"
    "id,username,password,email FROM users --%'"
)
pdf.n("结果：成功窃取所有用户明文密码！admin123, alice2025, test123")
pdf.img("/root/flask-app-vuln/static/ss_inject_union.png",w=170)

pdf.add_page()
pdf.h2("2.5 后台SQL日志截图")
pdf.t("Flask 控制台打印的实际执行的SQL语句，验证注入成功：")
pdf.img("/root/flask-app-vuln/static/ss_sql_log.png",w=170)

# ===== 三 =====
pdf.h1("三、漏洞危害分析")
pdf.t("本次发现的SQL注入漏洞危害：")
pdf.b("1. 数据泄露：UNION SELECT 可获取 users 表所有字段，包括密码")
pdf.b("2. 绕过认证：OR 1=1 永真条件绕过数据访问控制")
pdf.b("3. 数据篡改：注册功能注入点可修改或删除数据")
...
pdf.h2("4.1 参数化查询原理")
pdf.t("参数化查询将SQL结构和数据分离。数据库驱动自动转义和绑定参数，用户输入无法改变SQL语义。")
pdf.c(
    '# SAFE: ? placeholders + parameter tuple\n'
    'sql = "SELECT * FROM users WHERE username LIKE ? OR email LIKE ?"\n'
    'params = (f"%{keyword}%", f"%{keyword}%")\n'
    'c.execute(sql, params)  # SAFE!\n'
    '\n'
    '# Even if keyword = "%\' OR 1=1 --"\n'
    '# DB driver treats as plain text, NOT SQL code'
)
...
pdf.c(
    ...
    'params = (f"%{keyword}%", f"%{keyword}%")\n'
    'c.execute(sql, params)  # Parameterized!'
)
...
pdf.t("修改内容：搜索和注册的 f-string 拼接全部改为 ? 占位符参数化查询。")
pdf.c(
...
pdf.add_page()
pdf.h2("4.4 修复后SQL日志对比")
pdf.t("修复后，注入字符被当作普通文本搜索，不再作为SQL语句执行：")
pdf.img("/root/flask-app-vuln/static/ss_fixed_log.png",w=170) if False else pdf.t("（截图路径：/root/flask-app-vuln/static/ss_fixed_log.png）")
...
tests = [
    ...
    ("永真注入 %' OR 1=1", "http://127.0.0.1:5000/search?keyword=%25%27+OR+1%3D1+--", "无搜索结果"),
    ("UNION注入获取密码", "http://127.0.0.1:5000/search?keyword=%27+UNION+SELECT+id,username,password,email+FROM+users+--", "无搜索结果"),
]
...
pdf.b("UNION SELECT 不再泄露密码（返回无搜索结果）")
pdf.t("")
pdf.c(
    '# Fixed SQL log\n'
    "[SQL] SELECT ... WHERE username LIKE ? OR email LIKE ?\n"
...
pdf.t("")
pdf.t("项目文件：")
pdf.c("漏洞版: app.py (f-string拼接)")
pdf.c("修复版: app_fixed.py (参数化查询)")
